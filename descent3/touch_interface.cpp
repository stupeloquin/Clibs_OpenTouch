...
//
// Started from the Descent 1/2 layout, which it can be because the layout is
// written entirely in PORT_ACT_* codes - nothing in here knows which engine is
// listening. Descent 3 has its own extras to place later (external camera,
// countermeasures, the guidebot); this is the same 6DOF arrangement to begin
// with, so a player coming from the other ports finds their thumbs in the
// right places.
//
...
// The engine reads these as ordinary SDL input - see d3_game_interface.cpp.
//
...
{
    // One game, one layout - no runtime choice to make.
    return "descent3";
}
...
void TouchInterface::createControls(std::string filesPath)
{
    ...
    // Ship systems. Descent 3 has all of these, so none are hidden for being
    // unavailable - unlike the Descent 1/2 layout this started from.
    const bool hideD2 = false;
